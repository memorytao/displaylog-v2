...
const CONTACT_FIELDS = TOL_CONTACT_FIELD.split("|");
const TARGET_KEYS = ["AGW01", "AGW02", "AGW03", "AGW04"];

const TOLContactTable = ({ data, isLoading }) => {
    const rows = useMemo(() => {
    ...
    const renderCell = (cell, index) => {
        ...
        if (index === 4 || index === 5) return <span className="font-mono text-lg font-normal bg-gradient-to-r from-teal-400 to-yellow-300 bg-clip-text text-transparent">{cell}</span>;
        ...
        if (index === 24) return <span className="font-mono text-lg font-thin min-w-[300px] max-w-[500px] whitespace-normal break-words leading-relaxed text-yellow-300">{cell}</span>;
        if (index === 26) return <span className={`font-mono text-lg font-semibold ${cell === "ERROR" || strCell.toUpperCase() === "SUCCESS" ? "text-green-400" : "text-red-400"}`}>{cell}</span>;
        if (index === 19 || index === 20 || index === 29) return <span className="font-mono text-lg font-normal">{strCell.slice(0, 19)}</span>;
...
export default TOLContactTable;